...
app.controller("AppCtrl", ($scope) => {
    ...
    $appCtrl.GenerateApk = async (apkFolder) => {
        ...
        // NOTE: apktool 3.x removed the "empty-framework-dir" command (the framework cache is
        // managed automatically now), and the old call threw asynchronously anyway - the throw
        // happened inside the exec callback, i.e. outside this try/catch.

        // aapt2 cannot compile resources from a non-ASCII path (e.g. a Chinese folder name),
        // so copy the payload source to an ASCII temp dir (under dataPath) before building.
        var buildSrc = dir.join(dataPath, 'buildSrc');
        fs.removeSync(buildSrc);
        fs.copySync(apkFolder, buildSrc);
        fs.removeSync(dir.join(buildSrc, 'build'));
        // Build the AhMyth Payload APK
        delayedLog('[★] Building ' + CONSTANTS.apkName + '...');
        var createApk = 'java -jar "' + CONSTANTS.apktoolJar + '" b "' + buildSrc + '" -o "' + dir.join(outputPath,
            CONSTANTS.apkName) + '"';
        exec(createApk,
            (error, stdout, stderr) => {
                if (error !== null) {
                                        ...
                                        fs.readFile(dir.join(apkFolder, 'AndroidManifest.xml'), 'utf8', (error, data) => {
                                            ...
                                            fs.writeFile(dir.join(apkFolder, 'AndroidManifest.xml'), repXmlSdk, 'utf8', (error) => {
                                                ...
                                                fs.readFile(dir.join(apkFolder, 'apktool.yml'), 'utf8', (error, data) => {
                                                    ...
                                                    // apktool 2.x writes 'quoted' sdk values, 3.x writes bare ints - accept both, emit bare.
